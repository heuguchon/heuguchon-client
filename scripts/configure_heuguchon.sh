...
if [ $WANTS_CONFIG -eq $TRUE ] ; then
    ...
    if [ $WANTS_CRASHREPORTING -eq $TRUE ] ; then
        ...
        if [ ! -z $CHANNEL_SIMPLE ]
        then
            CRASH_REPORTING="$CRASH_REPORTING -DUSE_BUGSPLAT=On -DBUGSPLAT_DB=heuguchon_"`echo $CHANNEL_SIMPLE | tr [:upper:] [:lower:] | sed -e 's/x64//' | sed 's/[^A-Za-z0-9]//g'`
        fi
    else
...
if [ $WANTS_BUILD -eq $TRUE ] ; then
    ...
    if [ $TARGET_PLATFORM == "darwin" ] ; then
        ...
        if [ -f "Heuguchon.slnx" ]; then
          SOLUTION="Heuguchon.slnx"
        elif [ -f "Heuguchon.sln" ]; then
          SOLUTION="Heuguchon.sln"
        else
          echo "Build failed! No Heuguchon.slnx or Heuguchon.sln found"
          exit 1
        fi
        msbuild.exe "$SOLUTION" -p:Configuration=${BTYPE} -flp:LogFile="logs\\HeuguchonBuild_win-${AUTOBUILD_ADDRSIZE}.log" \
